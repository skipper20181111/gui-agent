...
- [2. GUI Agent 技术探索：以挂号场景为例（可快速迁移 Go/Java）](#2-gui-agent-技术探索以挂号场景为例可快速迁移-gojava)
  - [2.1 项目简介](#21-项目简介)
  ...
  - [2.6 工具集设计](#26-工具集设计)
  - [2.7 技术总结](#27-技术总结)
  - [2.8 延伸场景：Web 应用自动化测试](#28-延伸场景web-应用自动化测试)
...
蚂蚁阿福定位为 **AI 健康伴侣**，核心价值是"让你少生病、晚生病、更懂自己身体"。**母婴全周期健康管理** 与这一定位高度契合——从备孕到育儿，用户有持续的健康咨询需求和决策焦虑，是一个值得深耕的高频场景。
...
母婴场景中存在大量跨平台操作需求（挂号预约、政务办理、报告下载等），目前主要依赖用户手动完成，流程繁琐。

GUI Agent 作为自动化执行层，理论上可以与阿福协同：**阿福提供健康决策支持，GUI Agent 辅助执行落地**。这一方向值得持续探索，但当前技术仍处于早期阶段，存在以下局限：

| 局限性 | 说明 |
|--------|------|
| **效率较低** | 纯视觉方案需多轮滚动截图，Token 消耗高 |
| **异常处理弱** | 验证码、弹窗打断、网络超时等场景缺乏鲁棒性 |
| **移动端支持不足** | 当前方案基于桌面端，移动端需额外适配 |
| **安全风险** | 涉及账号、支付等敏感操作，需谨慎设计 |

待技术成熟后，可逐步落地到实际场景。

---

## 2. GUI Agent 技术探索：以挂号场景为例（可快速迁移 Go/Java）

### 2.1 项目简介

这是一个基于视觉的 GUI Agent 技术探索项目，使用 ReAct Agent 模式控制电脑屏幕。项目以医院挂号作为验证场景，探索 GUI Agent 在跨平台自动化操作中的可行性与局限性。

**探索方向**：
- **纯视觉方案**：验证跨平台通用性，支持任何 GUI 界面
- **ReAct Agent**：探索自主决策能力，多轮迭代完成任务
- **Token 优化**：尝试截图历史裁剪策略
...
- ✅ ReAct 循环逻辑简单，易于调试
- ✅ 完全可控，可灵活扩展
- ✅ **易于跨语言迁移**：核心逻辑仅 ~200 行，可快速改写为 Go/Java 版本，便于集成到现有后端系统
...
4. **精简工具集**：4 个核心工具，覆盖基础 GUI 操作

#### 2.7.3 已知局限与优化方向

| 局限 | 说明 | 可能的优化 |
|------|------|------------|
| **固定延迟** | 操作后固定等待 500ms，可能过长或过短 | 检测页面变化后再截图 |
| **无 DOM 支持** | 纯视觉效率低，滚动查找耗时长 | 生产环境优先使用 DOM，降级到视觉 |
| **异常处理弱** | 验证码、弹窗等场景未覆盖 | 增加异常检测与恢复机制 |
| **仅桌面端** | 基于 PyAutoGUI，不支持移动端 | 接入 Appium 或 ADB |

### 2.8 延伸场景：Web 应用自动化测试

GUI Agent 技术同样适用于 Web 应用的自动化测试场景，有望解决传统 E2E 测试的痛点：

#### 2.8.1 与传统 E2E 测试对比

| 维度 | 传统 E2E 测试 | GUI Agent 测试 |
|------|--------------|----------------|
| **定位方式** | 依赖 CSS 选择器/XPath，UI 重构即失效 | 基于视觉理解，UI 样式变化不影响 |
| **用例编写** | 需开发人员编写维护脚本 | 自然语言描述测试意图即可 |
| **验证层面** | 只能验证"元素存在" | 能验证"用户能否看到/理解" |
| **维护成本** | 高，选择器频繁失效 | 低，视觉理解更鲁棒 |

#### 2.8.2 潜在价值

| 价值 | 说明 |
|------|------|
| **降低维护成本** | 告别选择器地狱，UI 重构不再需要大规模修改测试代码 |
| **测试即文档** | 用自然语言描述用例，非技术人员也能理解测试意图 |
| **真实用户视角** | 从视觉层面发现可用性问题（如文字遮挡、按钮不可见等） |
| **跨平台复用** | 同一套测试逻辑可用于 Web、桌面、移动端 |
